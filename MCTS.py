...
def get_valid_input(board: GameBoard):
    while True:
        try:
            ...
            if (r, c) in board.getmoves():
                return (r, c)
            else:
                print("Invalid move, try that again.")
        except:
...
def play_game():
    ...
    while player_symbol not in ["1", "2"]:
        player_symbol = input("Invalid choice. Enter 1 for X or 2 for O")
    player_symbol = int(player_symbol)

    print("Game is starting! You are", "X" if player_symbol == 1 else "O")
    print_board(bd)

    while bd.checkwin() == 0:
        current_player = bd.check_nextplayer(bd.entries)

        if current_player == player_symbol:
            print("Your turn")
            r, c = get_valid_input(bd)
            bd.entries[r][c] = player_symbol
        else:
            print("Thinking....")
            action, _, bd = MCTS_move(bd, iterations=500)
            print("MCTS plays", action)
